            ...
            for j = 1:numMes
                ...
                for k = 1:numPsr
                    ...
                    if fCarr == 1
                        ...
                        % GPS Raw Observables
                        out.gpsRawObs(j).L1.prn(L1ctr) = double(msg{j}.Measurements(k).Prn);
                        out.gpsRawObs(j).L1.psr(L1ctr) = double(msg{j}.Measurements(k).Pseudorange);
                        out.gpsRawObs(j).L1.psr_variance(L1ctr) = double(msg{j}.Measurements(k).PseudorangeVariance);
                        out.gpsRawObs(j).L1.carr_phase(L1ctr) = double(msg{j}.Measurements(k).CarrierPhase);
                        out.gpsRawObs(j).L1.carr_phase_variance(L1ctr) = double(msg{j}.Measurements(k).CarrierPhaseVariance);
                        out.gpsRawObs(j).L1.dopp(L1ctr) = double(msg{j}.Measurements(k).Doppler);
                        out.gpsRawObs(j).L1.dopp_variance(L1ctr) = double(msg{j}.Measurements(k).DopplerVariance);
                        out.gpsRawObs(j).L1.cn0(L1ctr) = double(msg{j}.Measurements(k).CarrierToNoise);
                        out.gpsRawObs(j).L1.loss_of_lock(L1ctr) = double(msg{j}.Measurements(k).LossOfLock);
